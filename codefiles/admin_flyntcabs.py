...
def q6():
    ...
    earning = mycursor.fetchone()
    earnings=earning[0]
    if(earnings==None):
        earnings=0
    print("Revenue Generated : ",earnings)
...
def q7():
    ...
    WHERE b.bookingtime <= date_add(NOW(), INTERVAL 100 DAY) AND b.fare > (\
...
def q8():
    print("You selected Deletes cabs as well as drivers for car older than 2008")
    mydb = mysql.connector.connect(
    host="127.0.0.1",
    user="root",
    password="Chai@123",
...
def q10():
    print("You selected Driver details for those haven't been active (have not got a ride ) for a while and updating those as non active")
    mydb = mysql.connector.connect(
    host="127.0.0.1",
    user="root",
    password="Chai@123",
    ...
    mycursor = mydb.cursor()
    print("Time in days of your choice")
    input_u=int(input(""))
    ...
    WHERE b.bookingtime >= DATE_SUB(NOW(), INTERVAL "+str(input_u)+" DAY) \
...
def q11():
    ...
    mycursor = mydb.cursor()
    avg=int(input("Enter Avg more than what in the last 100 days"))
    mycursor.execute("SELECT u.FName, u.PrimaryPhone, b.bookingtime\
    FROM customer_records AS u\
    INNER JOIN Bookings AS b ON u.id = b.user_id\
    WHERE b.bookingtime <= date_add(NOW(), INTERVAL 100 DAY) AND b.fare > "+str(avg)+";")
    results = mycursor.fetchall()
    print("FName |"," PrimaryPhone |", " Booking Time")
    for row in results:
        print(row[0]," | ",row[1]," | ",row[2])
...
def q12():
    print("You selected Driver and Cab details for the driver who are currently online")
    mydb = mysql.connector.connect(
    host="127.0.0.1",
    user="root",
    password="Chai@123",
...
def q13():
    ...
    mycursor = mydb.cursor()
    mycursor.execute("create unique index i1 on customer_records (id, Fname);")
...
def q14():
    ...
    mycursor = mydb.cursor()
    mycursor.execute("create view user_view AS select id, Fname from customer_records; \
                     Grant Select on user_view")
...
def Normal():
    ...
    print("11. Enter Avg more than what in the last 100 days")
    print("12. Driver and Cab details for the driver who are currently online")
    print("13. Indexing on user")
    print("14. View for user records")
    print("15. Go back to previous menu")
    try:
        tt=int(input())
        if(tt==1):
            q1()

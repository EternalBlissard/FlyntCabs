...
def user_input(id):
    ...
    if choice == 1:
        ...
        total = 0.0
        for row in results:
            print("Month: %s, Total Spending: %s" %(row[0], row[1]))
            total+=float(row[1])
            print("Total spending for the last year: %s" % total)
    elif choice == 7:
        place=["Nehru Place","Lajpat Nagar","Jangpura","Central Secratait","CP","Govindpuri","Harkesh Nagar"]
        print("locations")
        for i in range(len(place)):
            print(str(i+1)+"   "+place[i])
        start=int(input("PickUP"))
        end=int(input("DropOff"))
        PickUp=place[start-1]
        DropOff=place[end-1]
        sql = "SELECT path from distance where place='"+ PickUp+"'"
        mycursor.execute(sql)
        results = mycursor.fetchall()
        sql_ = "SELECT path from distance where place='"+ DropOff+"'"
        mycursor.execute(sql_)
        results_ = mycursor.fetchall()
        distance=abs(results[0][0]-results_[0][0])

        sql1="SELECT cab_id from cabs where status='online';"
        mycursor.execute(sql1)
        results_ = mycursor.fetchall()
        if(len(results_)):
            driverid=results_[0][0]
            sql2="SELECT booking_id from bookings ;"
            mycursor.execute(sql2)
            results= mycursor.fetchall()
            bid=len(results)+1
            sql3="insert into bookings values("+str(bid)+","+str(id)+","+str(driverid)+","+"NOW(),"+str(distance*14)+")"
            mycursor.execute(sql3)
            mydb.commit()
            print("Booking Successful ! your fare is "+str(distance*14))
